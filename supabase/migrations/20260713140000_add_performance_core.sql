...
language plpgsql security definer stable set search_path=''as$$declare o public.deal_orders;q public.deal_quotes;paid numeric;reversed numeric;expenses numeric;begin
 ...
 net_customer_payments:=paid-reversed;internal_settlement_due:=o.internal_due;sales_expenses:=expenses;sales_margin:=net_customer_payments-internal_settlement_due-sales_expenses;return next;end$$;
...
returns public.performance_quarterly_targets language plpgsql security definer set search_path=''as$$
...
begin if p_points_target<0 or p_new_gmv_target<0 or p_renewal_gmv_target<0 or p_quarter_start<>date_trunc('quarter',p_quarter_start)::date then raise exception'INVALID_TARGET'using errcode='22023';end if;
 ...
 insert into public.audit_logs(team_id,actor_id,action,target_type,target_id,after_data)values(t.team_id,r.id,'performance.target_set','performance_target',t.id,to_jsonb(t));return t;end$$;

create or replace function public.update_estimated_points(p_quarter_start date,p_estimated_points numeric)
returns public.performance_quarterly_targets language plpgsql security definer set search_path=''as$$declare r public.profiles;t public.performance_quarterly_targets;begin
 ...
 insert into public.performance_target_events(team_id,target_id,event_type,snapshot,actor_id)values(t.team_id,t.id,'estimate_updated',to_jsonb(t),r.id);return t;end$$;

create or replace function public.create_official_reconciliation(p_quarter_start date,p_source_ref text,p_lines jsonb)
returns public.official_reconciliation_batches language plpgsql security definer set search_path=''as$$declare r public.profiles;b public.official_reconciliation_batches;line jsonb;begin
 ...
 for line in select*from jsonb_array_elements(p_lines)loop insert into public.official_reconciliation_lines(team_id,batch_id,profile_id,order_id,official_points,gmv_type,gmv_amount)
  values(r.team_id,b.id,(line->>'profile_id')::uuid,nullif(line->>'order_id','')::uuid,coalesce((line->>'official_points')::numeric,0),line->>'gmv_type',coalesce((line->>'gmv_amount')::numeric,0));end loop;return b;end$$;

create or replace function public.confirm_official_reconciliation(p_batch_id uuid)
returns public.official_reconciliation_batches language plpgsql security definer set search_path=''as$$declare r public.profiles;b public.official_reconciliation_batches;agg record;t public.performance_quarterly_targets;begin
 ...
 insert into public.audit_logs(team_id,actor_id,action,target_type,target_id,after_data)values(b.team_id,r.id,'reconciliation.confirmed','reconciliation_batch',b.id,to_jsonb(b));return b;end$$;
...
returns public.profit_adjustments language plpgsql security definer set search_path=''as$$declare r public.profiles;a public.profit_adjustments;begin
 ...
 insert into public.audit_logs(team_id,actor_id,action,target_type,target_id,after_data)values(a.team_id,r.id,'profit.adjustment_added','profit_adjustment',a.id,to_jsonb(a));return a;end$$;
