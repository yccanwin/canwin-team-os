...
begin
  ...
  with actions as(
    select 'lead_action:'||l.id::text id,l.id entity_id,'lead'::text entity_type,
      ...
      case when l.next_action_kind='appointment'and l.next_action_at<v_now then10 when l.next_action_kind='appointment'then30 when l.next_action_at<v_now then35 else50 end priority,
    ...
    select 'lead_recycle:'||l.id::text,l.id,'lead',case when(l.created_at at time zone'Asia/Shanghai')::date+2<=v_today then'recycle_48h'else'recycle_24h'end,
      case when(l.created_at at time zone'Asia/Shanghai')::date+2<=v_today then15 else40 end,
      ...
      ((((l.created_at at time zone'Asia/Shanghai')::date+case when(l.created_at at time zone'Asia/Shanghai')::date+2<=v_today then2 else1 end)::timestamp)at time zone'Asia/Shanghai'),'#/sales-v3?lead='||l.id::text,
    ...
    select 'renewal:'||rm.id::text,rm.id,'renewal','renewal_'||rm.days_before,
      case rm.days_before when15 then18 when30 then20 else22 end,case when rm.due_on<=v_today then'high'else'medium'end,
    ...
    select 'delivery:'||e.id::text,e.id,'delivery_exception',case when e.exception_type='stock_shortage'then'delivery_shortage'else'delivery_exception'end,
      case when e.exception_type='stock_shortage'then5 else25 end,case when e.exception_type='stock_shortage'then'critical'else'high'end,
