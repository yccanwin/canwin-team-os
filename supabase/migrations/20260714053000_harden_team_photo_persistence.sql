...
using (
  ...
  and (
    ...
    or public.has_role(team_id, array['admin', 'captain'])
  )
)
with check (
  team_id = 'CANWIN_TEAM'
...
-- The previous bucket policy allowed an active member to choose any path
-- below their team. Preserve legacy non-photo uploads, but explicitly remove
-- photos from that broad policy so the strict policy below is authoritative.
  ...
  and exists (
    ...
    where p.id = (select auth.uid())
      ...
      and (
        ...
        or public.has_role(p.team_id, array['admin', 'captain'])
      )
  )
);
