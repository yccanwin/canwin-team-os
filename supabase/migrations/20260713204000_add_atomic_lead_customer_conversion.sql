...
declare r public.profiles;l public.crm_leads;phone text;
begin
 ...
 select p.phone into phone from public.crm_lead_private p where p.lead_id=l.id and p.team_id=l.team_id;
...
declare r public.profiles;l public.crm_leads;b public.crm_brands;s public.crm_stores;c public.crm_contacts;cv public.crm_lead_conversions;phone text;store_owner uuid;phone_matches integer;
begin
 ...
 select p.phone into phone from public.crm_lead_private p where p.lead_id=l.id and p.team_id=l.team_id;if nullif(trim(phone),'')is null then raise exception'LEAD_PHONE_REQUIRED'using errcode='22023';end if;
