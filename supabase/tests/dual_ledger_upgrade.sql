...
begin
 ...
 if to_regprocedure('public.get_company_profit_summary()')is null or position('get_company_profit_summary' in company_view)=0 then raise exception'Secure company profit wrapper missing';end if;
 ...
 if position('has_access_role' in company_def)=0 or position('finance.read' in company_def)=0 or position('customers.supervise' in company_def)>0 then raise exception'Company function role filter unsafe';end if;
 ...
 if position('q.owner_id=auth.uid()' in lower(regexp_replace(pg_get_viewdef('public.personal_sales_margin'::regclass,true),'[[:space:]]+','','g')))=0 then raise exception'Sales own-margin isolation broken';end if;
