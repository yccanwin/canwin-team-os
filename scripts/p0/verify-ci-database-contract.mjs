...
function validate(candidate) {
  ...
  check(candidate.contractStatus === 'p1_acl_repair_parser_fix_remote_qualified_after_preserved_formal_dry_run_failure', 'contract status drift')
  ...
  check(boundary.aclRepairMigrationPath === 'supabase/migrations/20260720015435_harden_server_only_rpc_acl.sql' && boundary.aclRepairLocalGates === 'passed' && boundary.aclRepairDatabaseCi === 'passed-signed-run-29738966326' && boundary.formalAclRepairDryRun === 'failed-stop-preserved' && boundary.formalAclRepairFailureRunId === 'p1-acl-repair-20260720T104323349Z-4fa8de78a8' && boundary.aclRepairQualification === 'qualified-after-preserved-formal-dry-run-failure', 'ACL repair current boundary drift')
  check(boundary.aclRepairQualificationCount === 1 && boundary.aclRepairQualifiedRunId === '29738966326' && boundary.aclRepairRemoteExecutionAllowed === true && boundary.aclRepairDbPushAllowed === true, 'ACL repair one-shot qualification boundary drift')
  ...
  check(aclRepair.currentCiQualification === 'qualified-signed-run-29738966326' && aclRepair.priorSuccessfulCiEvidenceRef === 'priorSuccessfulRepairCiRunEvidence' && aclRepair.signedCiHeadSha === '070c2e4ca185037d37f65b4d98be617a43e4409d' && aclRepair.signedCiRunId === '29738966326' && aclRepair.signedCiLinuxJobId === '88340968144' && aclRepair.signedCiWindowsJobId === '88340968119' && aclRepair.signedCiConclusion === 'success', 'signed ACL repair parser-fix CI identity drift')
  ...
  check(JSON.stringify(currentRuntime.contract) === JSON.stringify({ path: 'scripts/p1/isolated-runtime-contract.json', sha256Lf: '80663687bb68292af3d4febca682b0a981562efee48788fe07d1bc75c283120e' }) && currentRuntime.contract.sha256Lf === sha256Lf(resolve(repoRoot, currentRuntime.contract.path)), 'ACL repair current runtime contract SHA drift')
  check(JSON.stringify(currentRuntime.runner) === JSON.stringify({ path: 'scripts/p1/run-isolated-runtime.mjs', sha256Lf: '8ffd152c34aa4f5f3ad4b297670d4ee6a6556c83da9a1d8f38c006c2b2c76d95' }) && currentRuntime.runner.sha256Lf === sha256Lf(resolve(repoRoot, currentRuntime.runner.path)), 'ACL repair current runner SHA drift')
  check(JSON.stringify(currentRuntime.validator) === JSON.stringify({ path: 'scripts/p1/verify-isolated-runtime-runner.mjs', sha256Lf: '68b4ee999ab372790a149a7a0fc9c1b3e3db618805dabc8117434e0a092d955e', assertions: '68/68' }) && currentRuntime.validator.sha256Lf === sha256Lf(resolve(repoRoot, currentRuntime.validator.path)), 'ACL repair current validator SHA or assertion drift')
  ...
  check(formalAclRepairFailure.runId === 'p1-acl-repair-20260720T104323349Z-4fa8de78a8' && formalAclRepairFailure.evidenceDirectory === 'D:/CanWin-Team-OS-4.0-P1-Validation/p1-acl-repair-20260720T104323349Z-4fa8de78a8' && formalAclRepairFailure.failurePath === 'D:/CanWin-Team-OS-4.0-P1-Validation/p1-acl-repair-20260720T104323349Z-4fa8de78a8/failure.json', 'formal ACL repair failure evidence identity drift')
  check(formalAclRepairFailure.failureSha256 === '16373794dd745ad86422bb59f3966933532cb0bf073251963b519c2b8e367e73' && formalAclRepairFailure.supervisionHeadSha === '4fa8de78a8b05f8285f69fb0d6d9106e20e3cba7', 'formal ACL repair failure evidence SHA or head drift')
  check(formalAclRepairFailure.status === 'failed-stop-preserved' && formalAclRepairFailure.currentStep === 'db-push-dry-run' && formalAclRepairFailure.message === 'ACL repair dry-run did not enumerate exactly the one signed migration 71' && formalAclRepairFailure.migrationVersion === '20260720015435' && formalAclRepairFailure.migrationAlreadyApplied === false, 'formal ACL repair failure result drift')
  ...
  check(formalAclRepairFailure.formalAttemptStarted === false && formalAclRepairFailure.dbPushAttempted === false && formalAclRepairFailure.dbPushPerformed === false && formalAclRepairFailure.dbPushAttempts === 0 && formalAclRepairFailure.attempts === 0, 'formal ACL repair attempt boundary drift')
  check(formalAclRepairFailure.confirmedPersistentWrites === 0 && formalAclRepairFailure.persistentRemoteWrites === 0 && formalAclRepairFailure.persistentRemoteWriteUpperBound === 0 && formalAclRepairFailure.productionReads === 0 && formalAclRepairFailure.productionWrites === 0, 'formal ACL repair write boundary drift')
  check(formalAclRepairFailure.targetPreserved === true && formalAclRepairFailure.retryPerformed === false && formalAclRepairFailure.remoteCleanupPerformed === false && JSON.stringify(formalAclRepairFailure.evidenceInventory) === JSON.stringify({ files: ['failure.json'], fileCount: 1, onlyFailureJson: true }) && formalAclRepairFailure.successEvidencePresent === false, 'formal ACL repair failure preservation drift')

  const repairCi = candidate.repairCiRunEvidence ?? {}
  check(repairCi.status === 'success' && repairCi.conclusion === 'success' && repairCi.runId === '29738966326' && repairCi.runUrl === 'https://github.com/yccanwin/canwin-team-os/actions/runs/29738966326' && repairCi.headSha === '070c2e4ca185037d37f65b4d98be617a43e4409d' && repairCi.linuxJobId === '88340968144' && repairCi.windowsJobId === '88340968119', 'current ACL repair parser-fix CI identity drift')
  check(repairCi.linuxStatus === 'success' && repairCi.windowsStatus === 'success' && repairCi.qualificationScope === 'acl_repair_parser_fix_prequalification' && repairCi.linuxDatabaseAccepted === true, 'current ACL repair parser-fix CI platform result drift')
  check(repairCi.migrationsPassed === 71 && repairCi.sqlTestsStarted === 27 && repairCi.sqlTestsPassed === 27 && repairCi.databaseTestsPassed === 7 && repairCi.permissionTestsPassed === 11 && repairCi.businessTestsPassed === 9 && repairCi.catalogAssertionsPassed === 4, 'current ACL repair parser-fix CI database counts drift')
  check(repairCi.windowsStaticExpected === 19 && repairCi.windowsStaticPassed === 19 && repairCi.windowsStaticGatesExpected === 19 && repairCi.windowsStaticGatesPassed === 19 && repairCi.windowsLocalExpected === 12 && repairCi.windowsLocalPassed === 12 && repairCi.windowsLocalIntegrationStepsExpected === 12 && repairCi.windowsLocalIntegrationStepsPassed === 12, 'current ACL repair parser-fix CI Windows counts drift')
  check(repairCi.cleanupPassed === true && repairCi.repositorySecretsRequired === false && repairCi.repositorySecretsUsed === 0 && repairCi.testProjectRemoteReads === 0 && repairCi.testProjectRemoteWrites === 0 && repairCi.productionReadPerformed === false && repairCi.productionWritePerformed === false && repairCi.retryPerformed === false, 'current ACL repair parser-fix CI safety evidence drift')
  check(repairCi.priorFailedRunPreservedWithoutRerun === '29726897764' && repairCi.priorSuccessfulRunPreservedWithoutRerun === '29733854344' && repairCi.formalAclRepairFailurePreservedWithoutRerun === 'p1-acl-repair-20260720T104323349Z-4fa8de78a8' && repairCi.candidateRemoteExecutionAllowed === false, 'current ACL repair parser-fix history preservation drift')
  check(repairCi.databaseCiPassed === true && repairCi.remoteQualificationAllowed === true && repairCi.currentQualificationAllowed === true && repairCi.successEvidencePresent === true && repairCi.g1OverallClaim === false, 'current ACL repair parser-fix qualification drift')
