...
function validate(candidate) {
  ...
  check(candidate.contractStatus === 'p1_acl_and_atomic_compatibility_repair_candidate_remote_qualified', 'contract status drift')
  ...
  check(boundary.aclRepairMigrationPath === 'supabase/migrations/20260720015435_harden_server_only_rpc_acl.sql' && boundary.aclRepairLocalGates === 'passed' && boundary.aclRepairDatabaseCi === 'passed-signed-run-29733854344', 'ACL repair current boundary drift')
  ...
  check(aclRepair.signedCiHeadSha === '71b7320b4c303af797ee9e4bf12044518a4fe18a' && aclRepair.signedCiRunId === '29733854344' && aclRepair.signedCiLinuxJobId === '88324427055' && aclRepair.signedCiWindowsJobId === '88324427244' && aclRepair.signedCiConclusion === 'success', 'signed ACL repair CI identity drift')
  ...
  check(JSON.stringify(currentRuntime.contract) === JSON.stringify({ path: 'scripts/p1/isolated-runtime-contract.json', sha256Lf: '4b7db4155637bc349c50948eaa35f2d241e823b3e65c0578e3be083e93344659' }) && currentRuntime.contract.sha256Lf === sha256Lf(resolve(repoRoot, currentRuntime.contract.path)), 'ACL repair current runtime contract SHA drift')
  check(JSON.stringify(currentRuntime.runner) === JSON.stringify({ path: 'scripts/p1/run-isolated-runtime.mjs', sha256Lf: 'acae6010b6e4efca94d8411b16aa2cf8e1e04118ecf9055a9711ebb55eb3d2bc' }) && currentRuntime.runner.sha256Lf === sha256Lf(resolve(repoRoot, currentRuntime.runner.path)), 'ACL repair current runner SHA drift')
  check(JSON.stringify(currentRuntime.validator) === JSON.stringify({ path: 'scripts/p1/verify-isolated-runtime-runner.mjs', sha256Lf: '1aa9341425465a42ae7cd82d434281595723b00733fafc17be3176f73ee5d5f4', assertions: '63/63' }) && currentRuntime.validator.sha256Lf === sha256Lf(resolve(repoRoot, currentRuntime.validator.path)), 'ACL repair current validator SHA or assertion drift')
  ...
  check(repairCi.runId === '29733854344' && repairCi.runUrl === 'https://github.com/yccanwin/canwin-team-os/actions/runs/29733854344' && repairCi.headSha === '71b7320b4c303af797ee9e4bf12044518a4fe18a' && repairCi.linuxJobId === '88324427055' && repairCi.windowsJobId === '88324427244', 'successful ACL repair CI identity drift')
  check(repairCi.conclusion === 'success' && repairCi.linuxConclusion === 'success' && repairCi.windowsConclusion === 'success' && repairCi.linuxDatabaseAccepted === true, 'successful ACL repair CI platform result drift')
  check(repairCi.migrationsPassed === 71 && repairCi.sqlTestsStarted === 27 && repairCi.sqlTestsPassed === 27 && repairCi.databaseTestsPassed === 7 && repairCi.permissionTestsPassed === 11 && repairCi.businessTestsPassed === 9 && repairCi.catalogAssertionsPassed === 4, 'successful ACL repair CI database counts drift')
  check(repairCi.windowsStaticGatesExpected === 19 && repairCi.windowsStaticGatesPassed === 19 && repairCi.windowsLocalIntegrationStepsExpected === 12 && repairCi.windowsLocalIntegrationStepsPassed === 12, 'successful ACL repair CI Windows counts drift')
  check(repairCi.cleanupPassed === true && repairCi.repositorySecretsRequired === false && repairCi.productionReadPerformed === false && repairCi.productionWritePerformed === false && repairCi.rerunPerformed === false && repairCi.priorFailedRunPreservedWithoutRerun === '29726897764' && repairCi.candidateRemoteExecutionAllowed === false && repairCi.g1OverallClaim === false, 'successful ACL repair CI safety boundary drift')
...
const negativeCases = [
  ...
  ['successful ACL repair CI erased', (value) => { value.repairCiRunEvidence.conclusion = 'failure' }],
