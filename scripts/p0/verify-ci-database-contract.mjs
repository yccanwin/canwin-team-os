...
function validate(candidate) {
  ...
  check(candidate.contractStatus === 'p1_acl_repair_formal_dry_run_failed_qualification_closed', 'contract status drift')
  ...
  check(boundary.aclRepairMigrationPath === 'supabase/migrations/20260720015435_harden_server_only_rpc_acl.sql' && boundary.aclRepairLocalGates === 'passed' && boundary.aclRepairDatabaseCi === 'prior-successful-run-29733854344-historical-only-current-pending' && boundary.formalAclRepairDryRun === 'failed-stop-preserved' && boundary.formalAclRepairFailureRunId === 'p1-acl-repair-20260720T104323349Z-4fa8de78a8' && boundary.aclRepairQualification === 'closed-pending-new-signed-evidence', 'ACL repair current boundary drift')
  ...
  check(aclRepair.currentCiQualification === 'pending-new-signed-run' && aclRepair.priorSuccessfulCiEvidenceRef === 'priorSuccessfulRepairCiRunEvidence' && aclRepair.signedCiHeadSha === null && aclRepair.signedCiRunId === null && aclRepair.signedCiLinuxJobId === null && aclRepair.signedCiWindowsJobId === null && aclRepair.signedCiConclusion === null, 'pending signed ACL repair CI boundary drift')
  ...
  check(JSON.stringify(currentRuntime.contract) === JSON.stringify({ path: 'scripts/p1/isolated-runtime-contract.json', sha256Lf: '7d8a4c18dd67ef0d370509e7ac2707a475ecac08f766c447061bd768417f9ec3' }) && currentRuntime.contract.sha256Lf === sha256Lf(resolve(repoRoot, currentRuntime.contract.path)), 'ACL repair current runtime contract SHA drift')
  check(JSON.stringify(currentRuntime.runner) === JSON.stringify({ path: 'scripts/p1/run-isolated-runtime.mjs', sha256Lf: '3834ada48a3dd827dbb7d3ff15b0f5a068e2fa4daee4f93296e614990799d541' }) && currentRuntime.runner.sha256Lf === sha256Lf(resolve(repoRoot, currentRuntime.runner.path)), 'ACL repair current runner SHA drift')
  check(JSON.stringify(currentRuntime.validator) === JSON.stringify({ path: 'scripts/p1/verify-isolated-runtime-runner.mjs', sha256Lf: '2cf0ed22cb1d1a67179af78c24decf5dd8adad2fe8251f47acf1b8aa85e42c74', assertions: '68/68' }) && currentRuntime.validator.sha256Lf === sha256Lf(resolve(repoRoot, currentRuntime.validator.path)), 'ACL repair current validator SHA or assertion drift')
  ...
  check(repairCi.status === 'pending-new-signed-run' && repairCi.runId === null && repairCi.runUrl === null && repairCi.headSha === null && repairCi.linuxJobId === null && repairCi.windowsJobId === null && repairCi.conclusion === null, 'current ACL repair CI pending identity drift')
  check(repairCi.databaseCiPassed === null && repairCi.remoteQualificationAllowed === false && repairCi.successEvidencePresent === false && repairCi.closedByFormalAclRepairFailureRunId === 'p1-acl-repair-20260720T104323349Z-4fa8de78a8', 'current ACL repair CI closed qualification drift')

  const attempts = candidate.formalAttemptHistory ?? []
  check(attempts.length === 26, 'formal attempt history count drift')
...
const negativeCases = [
  ...
  ['atomic current database CI falsely accepted', (value) => { value.aclRepairCandidate.atomicLegacyRoleCompatibility.databaseCiPassed = true }],
  ...
  ['ACL repair db push falsely reopened', (value) => { value.aclRepairCandidate.dbPushAllowed = true }],
  ...
  ['current ACL repair CI falsely accepted', (value) => { value.repairCiRunEvidence.status = 'success' }],
  ...
  ['ACL repair acceptance boundary falsely reopened', (value) => { value.acceptanceBoundary.aclRepairQualification = 'open' }],
