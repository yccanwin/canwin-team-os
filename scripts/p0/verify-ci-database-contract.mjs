...
function validate(candidate) {
  ...
  check(candidate.contractStatus === 'p1_acl_repair_direct_db_dry_run_timeout_qualification_closed', 'contract status drift')
  ...
  check(boundary.aclRepairMigrationPath === 'supabase/migrations/20260720015435_harden_server_only_rpc_acl.sql' && boundary.aclRepairLocalGates === 'passed' && boundary.aclRepairDatabaseCi === 'prior-parser-fix-run-29738966326-historical-only-current-pending' && boundary.formalAclRepairDryRun === 'direct-db-connection-timeout-failed-stop-preserved' && boundary.formalAclRepairFailureRunId === 'p1-acl-repair-20260720T122757275Z-8fa1498850' && boundary.aclRepairQualification === 'closed-session-pooler-and-new-ci-required', 'ACL repair current boundary drift')
  check(boundary.aclRepairQualificationCount === 0 && boundary.aclRepairQualifiedRunId === null && boundary.aclRepairRemoteExecutionAllowed === false && boundary.aclRepairDbPushAllowed === false && boundary.nextAclRepairConnectionMode === 'session-pooler' && boundary.nextAclRepairIndependentCiRequired === true, 'ACL repair Session Pooler pending qualification boundary drift')
  ...
  check(aclRepair.mode === '--apply-acl-repair' && aclRepair.remoteExecutionAllowed === false && aclRepair.dbPushAllowed === false, 'ACL repair closed mode drift')
  ...
  check(aclRepair.currentCiQualification === 'pending-session-pooler-new-signed-run' && aclRepair.priorSuccessfulCiEvidenceRef === 'priorSuccessfulRepairCiRunEvidence' && aclRepair.priorParserFixCiEvidenceRef === 'priorParserFixRepairCiRunEvidence' && aclRepair.requiredConnectionMode === 'session-pooler' && aclRepair.newIndependentCiRequired === true && aclRepair.signedCiHeadSha === null && aclRepair.signedCiRunId === null && aclRepair.signedCiLinuxJobId === null && aclRepair.signedCiWindowsJobId === null && aclRepair.signedCiConclusion === null, 'pending Session Pooler ACL repair CI boundary drift')
  ...
  check(JSON.stringify(currentRuntime.contract) === JSON.stringify({ path: 'scripts/p1/isolated-runtime-contract.json', sha256Lf: 'e6b152e9dd98f848ceeac862669a3447e81a87fdc75772b5f027dbef6d15cca3' }) && currentRuntime.contract.sha256Lf === sha256Lf(resolve(repoRoot, currentRuntime.contract.path)), 'ACL repair current runtime contract SHA drift')
  check(JSON.stringify(currentRuntime.runner) === JSON.stringify({ path: 'scripts/p1/run-isolated-runtime.mjs', sha256Lf: '417565306ac5b515e11a009b85bc4f615ac99511dd4efc8a2ca65d5f2ee59061' }) && currentRuntime.runner.sha256Lf === sha256Lf(resolve(repoRoot, currentRuntime.runner.path)), 'ACL repair current runner SHA drift')
  check(JSON.stringify(currentRuntime.validator) === JSON.stringify({ path: 'scripts/p1/verify-isolated-runtime-runner.mjs', sha256Lf: 'bb8c7db55512db69b1d0451cc27c5a2b8532b6afecaf579023cfbed79a54a40a', assertions: '71/71' }) && currentRuntime.validator.sha256Lf === sha256Lf(resolve(repoRoot, currentRuntime.validator.path)), 'ACL repair current validator SHA or assertion drift')
  ...
  check(atomicCompatibility.status === 'passed' && atomicCompatibility.staticPassed === true && atomicCompatibility.databaseCiPassed === null && atomicCompatibility.remoteQualificationAllowed === false, 'atomic compatibility qualification drift')
  ...
  check(aclCompatibility.status === 'passed' && aclCompatibility.remoteQualificationAllowed === false && JSON.stringify(aclCompatibility.legacyRpcCallSites) === JSON.stringify([]), 'ACL repair application compatibility qualification drift')
  ...
  check(repairCi.status === 'pending-session-pooler-new-signed-run' && repairCi.conclusion === null && repairCi.runId === null && repairCi.runUrl === null && repairCi.headSha === null && repairCi.linuxJobId === null && repairCi.windowsJobId === null, 'current Session Pooler ACL repair CI pending identity drift')
  check(repairCi.qualificationScope === 'acl_repair_session_pooler_prequalification' && repairCi.requiredConnectionMode === 'session-pooler' && repairCi.newIndependentCiRequired === true && repairCi.databaseCiPassed === null && repairCi.remoteQualificationAllowed === false && repairCi.currentQualificationAllowed === false && repairCi.successEvidencePresent === false && repairCi.priorQualifiedRunId === '29738966326' && repairCi.closedByFormalAclRepairFailureRunId === 'p1-acl-repair-20260720T122757275Z-8fa1498850' && repairCi.g1OverallClaim === false, 'current Session Pooler ACL repair CI closed qualification drift')

  const attempts = candidate.formalAttemptHistory ?? []
  check(attempts.length === 27, 'formal attempt history count drift')
...
const negativeCases = [
  ...
  ['ACL repair remote qualification falsely reopened', (value) => { value.aclRepairCandidate.remoteExecutionAllowed = true }],
  ['ACL repair db push qualification falsely reopened', (value) => { value.aclRepairCandidate.dbPushAllowed = true }],
  ...
  ['ACL repair acceptance remote qualification falsely reopened', (value) => { value.acceptanceBoundary.aclRepairRemoteExecutionAllowed = true }],
  ['ACL repair acceptance db push qualification falsely reopened', (value) => { value.acceptanceBoundary.aclRepairDbPushAllowed = true }],
