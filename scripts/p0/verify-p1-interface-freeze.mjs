...
function validate(candidate) {
  ...
  check(JSON.stringify(currentRuntime.contract) === JSON.stringify({ path: 'scripts/p1/isolated-runtime-contract.json', sha256Lf: '7d8a4c18dd67ef0d370509e7ac2707a475ecac08f766c447061bd768417f9ec3' }) && currentRuntime.contract.sha256Lf === sha256Utf8Lf(readFileSync(resolve(repoRoot, currentRuntime.contract.path), 'utf8')), 'ACL repair current runtime contract SHA drift')
  check(JSON.stringify(currentRuntime.runner) === JSON.stringify({ path: 'scripts/p1/run-isolated-runtime.mjs', sha256Lf: '3834ada48a3dd827dbb7d3ff15b0f5a068e2fa4daee4f93296e614990799d541' }) && currentRuntime.runner.sha256Lf === sha256Utf8Lf(readFileSync(resolve(repoRoot, currentRuntime.runner.path), 'utf8')), 'ACL repair current runner SHA drift')
  check(JSON.stringify(currentRuntime.validator) === JSON.stringify({ path: 'scripts/p1/verify-isolated-runtime-runner.mjs', sha256Lf: '2cf0ed22cb1d1a67179af78c24decf5dd8adad2fe8251f47acf1b8aa85e42c74', assertions: '68/68' }) && currentRuntime.validator.sha256Lf === sha256Utf8Lf(readFileSync(resolve(repoRoot, currentRuntime.validator.path), 'utf8')), 'ACL repair current validator SHA or assertion drift')
  ...
  check(aclRepairCi.status === 'pending-new-signed-run' && aclRepairCi.runId === null && aclRepairCi.runUrl === null && aclRepairCi.headSha === null && aclRepairCi.linuxJobId === null && aclRepairCi.windowsJobId === null && aclRepairCi.conclusion === null, 'current ACL repair CI pending identity drift')
  check(aclRepairCi.databaseCiPassed === null && aclRepairCi.remoteQualificationAllowed === false && aclRepairCi.successEvidencePresent === false && aclRepairCi.closedByFormalAclRepairFailureRunId === 'p1-acl-repair-20260720T104323349Z-4fa8de78a8', 'current ACL repair CI closed qualification drift')
