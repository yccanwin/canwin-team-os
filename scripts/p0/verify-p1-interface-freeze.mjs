...
function validate(candidate) {
  ...
  check(JSON.stringify(currentRuntime.contract) === JSON.stringify({ path: 'scripts/p1/isolated-runtime-contract.json', sha256Lf: 'e6b152e9dd98f848ceeac862669a3447e81a87fdc75772b5f027dbef6d15cca3' }) && currentRuntime.contract.sha256Lf === sha256Utf8Lf(readFileSync(resolve(repoRoot, currentRuntime.contract.path), 'utf8')), 'ACL repair current runtime contract SHA drift')
  check(JSON.stringify(currentRuntime.runner) === JSON.stringify({ path: 'scripts/p1/run-isolated-runtime.mjs', sha256Lf: '417565306ac5b515e11a009b85bc4f615ac99511dd4efc8a2ca65d5f2ee59061' }) && currentRuntime.runner.sha256Lf === sha256Utf8Lf(readFileSync(resolve(repoRoot, currentRuntime.runner.path), 'utf8')), 'ACL repair current runner SHA drift')
  check(JSON.stringify(currentRuntime.validator) === JSON.stringify({ path: 'scripts/p1/verify-isolated-runtime-runner.mjs', sha256Lf: 'bb8c7db55512db69b1d0451cc27c5a2b8532b6afecaf579023cfbed79a54a40a', assertions: '71/71' }) && currentRuntime.validator.sha256Lf === sha256Utf8Lf(readFileSync(resolve(repoRoot, currentRuntime.validator.path), 'utf8')), 'ACL repair current validator SHA or assertion drift')
  ...
  const priorAclRepairCi = candidate.priorRepairCiFailureEvidence ?? {}
  check(priorAclRepairCi.runId === '29726897764' && priorAclRepairCi.headSha === 'e774ead5a2857afb511400a12897e629033cf941' && priorAclRepairCi.linuxJobId === '88301987239' && priorAclRepairCi.windowsJobId === '88301987280', 'prior failed ACL repair CI identity drift')
  check(priorAclRepairCi.conclusion === 'failure' && priorAclRepairCi.linuxConclusion === 'failure' && priorAclRepairCi.windowsConclusion === 'success' && priorAclRepairCi.sqlTestsStarted === 18 && priorAclRepairCi.sqlTestsPassed === 17, 'prior failed ACL repair CI result drift')
  check(priorAclRepairCi.failedAssertionExpectedAuditRows === 6 && priorAclRepairCi.failedAssertionActualAuditRows === 7 && JSON.stringify(priorAclRepairCi.correctedExpectedAuditRowsByAction) === JSON.stringify({ memberAccess: 5, supervisorSystem: 1, supervisorScope: 1 }) && priorAclRepairCi.preservedWithoutRerun === true, 'prior failed ACL repair audit evidence drift')
  ...
  check(priorParserFixRepairCi.status === 'success' && priorParserFixRepairCi.conclusion === 'success' && priorParserFixRepairCi.runId === '29738966326' && priorParserFixRepairCi.runUrl === 'https://github.com/yccanwin/canwin-team-os/actions/runs/29738966326' && priorParserFixRepairCi.headSha === '070c2e4ca185037d37f65b4d98be617a43e4409d' && priorParserFixRepairCi.linuxJobId === '88340968144' && priorParserFixRepairCi.windowsJobId === '88340968119' && priorParserFixRepairCi.qualificationScope === 'acl_repair_parser_fix_prequalification' && priorParserFixRepairCi.databaseCiPassed === true && priorParserFixRepairCi.remoteQualificationAllowed === true && priorParserFixRepairCi.currentQualificationAllowed === false && priorParserFixRepairCi.evidenceScope === 'historical-parser-fix-ci-for-failed-direct-db-candidate' && priorParserFixRepairCi.g1OverallClaim === false, 'historical parser-fix ACL repair CI drift')

  const aclRepairCi = candidate.repairCiRunEvidence ?? {}
  check(aclRepairCi.status === 'pending-session-pooler-new-signed-run' && aclRepairCi.conclusion === null && aclRepairCi.runId === null && aclRepairCi.runUrl === null && aclRepairCi.headSha === null && aclRepairCi.linuxJobId === null && aclRepairCi.windowsJobId === null, 'current Session Pooler ACL repair CI pending identity drift')
  check(aclRepairCi.qualificationScope === 'acl_repair_session_pooler_prequalification' && aclRepairCi.requiredConnectionMode === 'session-pooler' && aclRepairCi.newIndependentCiRequired === true && aclRepairCi.databaseCiPassed === null && aclRepairCi.remoteQualificationAllowed === false && aclRepairCi.currentQualificationAllowed === false && aclRepairCi.successEvidencePresent === false && aclRepairCi.priorQualifiedRunId === '29738966326' && aclRepairCi.closedByFormalAclRepairFailureRunId === 'p1-acl-repair-20260720T122757275Z-8fa1498850' && aclRepairCi.g1OverallClaim === false, 'current Session Pooler ACL repair CI closed qualification drift')
