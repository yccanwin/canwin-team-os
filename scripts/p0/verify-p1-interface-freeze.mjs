...
function validate(candidate) {
  ...
  check(JSON.stringify(currentRuntime.contract) === JSON.stringify({ path: 'scripts/p1/isolated-runtime-contract.json', sha256Lf: '0c0ccc0f72775ee1e6c6d46771846de88c5054a20b3f75b30a77db40ced08991' }) && currentRuntime.contract.sha256Lf === sha256Utf8Lf(readFileSync(resolve(repoRoot, currentRuntime.contract.path), 'utf8')), 'ACL repair current runtime contract SHA drift')
  check(JSON.stringify(currentRuntime.runner) === JSON.stringify({ path: 'scripts/p1/run-isolated-runtime.mjs', sha256Lf: '690dd490c4c66c84ed1d01654c77cdb6cf8ca068b6625c9f44ed78e32c0098ad' }) && currentRuntime.runner.sha256Lf === sha256Utf8Lf(readFileSync(resolve(repoRoot, currentRuntime.runner.path), 'utf8')), 'ACL repair current runner SHA drift')
  check(JSON.stringify(currentRuntime.validator) === JSON.stringify({ path: 'scripts/p1/verify-isolated-runtime-runner.mjs', sha256Lf: 'bc237b9b35ca6954a9c4cd6e905ff7a02608ad3509fcd439ba64a5a4882b8da3', assertions: '63/63' }) && currentRuntime.validator.sha256Lf === sha256Utf8Lf(readFileSync(resolve(repoRoot, currentRuntime.validator.path), 'utf8')), 'ACL repair current validator SHA or assertion drift')
  ...
  check(aclRepairCi.runId === '29726897764' && aclRepairCi.runUrl === 'https://github.com/yccanwin/canwin-team-os/actions/runs/29726897764' && aclRepairCi.headSha === 'e774ead5a2857afb511400a12897e629033cf941' && aclRepairCi.linuxJobId === '88301987239' && aclRepairCi.windowsJobId === '88301987280', 'failed ACL repair CI identity drift')
  check(aclRepairCi.conclusion === 'failure' && aclRepairCi.linuxConclusion === 'failure' && aclRepairCi.windowsConclusion === 'success' && aclRepairCi.failedStep === 'Run database permission and business gates', 'failed ACL repair CI platform result drift')
  check(aclRepairCi.firstFailedSqlTest === 'supabase/tests/team_os_4_p1_access_shell.sql' && aclRepairCi.rootCauseCode === 'p1_audit_expected_aggregate_6_actual_7' && aclRepairCi.rootCauseStatus === 'confirmed' && aclRepairCi.firstError === 'P1 access mutations did not create exact audit evidence: 7', 'failed ACL repair CI root cause drift')
  check(aclRepairCi.migrationsPassed === 71 && aclRepairCi.sqlTestsStarted === 18 && aclRepairCi.sqlTestsPassed === 17 && aclRepairCi.databaseTestsPassed === 7 && aclRepairCi.permissionTestsPassed === 10 && aclRepairCi.businessTestsPassed === 0 && aclRepairCi.catalogAssertionsPassed === 0, 'failed ACL repair CI database counts drift')
  check(aclRepairCi.failedAssertionExpectedAuditRows === 6 && aclRepairCi.failedAssertionActualAuditRows === 7 && JSON.stringify(aclRepairCi.correctedExpectedAuditRowsByAction) === JSON.stringify({ memberAccess: 5, supervisorSystem: 1, supervisorScope: 1 }), 'failed ACL repair audit evidence drift')
  check(aclRepairCi.windowsStaticGatesExpected === 19 && aclRepairCi.windowsStaticGatesPassed === 19 && aclRepairCi.windowsLocalIntegrationStepsExpected === 12 && aclRepairCi.windowsLocalIntegrationStepsPassed === 12, 'failed ACL repair CI Windows success counts drift')
  check(aclRepairCi.cleanupPassed === true && aclRepairCi.repositorySecretsRequired === false && aclRepairCi.productionReadPerformed === false && aclRepairCi.productionWritePerformed === false && aclRepairCi.rerunPerformed === false && aclRepairCi.preservedWithoutRerun === true && aclRepairCi.candidateRemoteExecutionAllowed === false && aclRepairCi.g1OverallClaim === false, 'failed ACL repair CI safety boundary drift')
