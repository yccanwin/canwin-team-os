...
> 状态：完整 catalog 原始快照尚未冻结；103 表和 162 函数的标准化元数据已于 2026-07-19 只读取证
> SQL：`scripts/p0/catalog-snapshot.sql`、`scripts/p0/public-table-live-evidence.sql`、`scripts/p0/public-routine-live-evidence.sql`、`scripts/p0/public-foreign-key-risk-live-evidence.sql`
...
当前逐表、逐函数和外键风险 SQL 已分别通过 Supabase 只读查询执行，标准化结果保存在对应 JSON。逐表证据覆盖 103/103 表及229策略、29触发器、248索引、309外键；逐函数证据登记162/162签名；外键风险证据把309个外键精确到源/目标字段，并证明Advisor口径覆盖104、未覆盖205。三类证据的业务行读取=0、写入=0，函数正文返回=0；完整函数正文依赖、策略表达式和原始连接输出仍不进入仓库。
...
powershell.exe -NoProfile -ExecutionPolicy Bypass -File .\scripts\p0\validate-catalog-snapshot-readonly.ps1 -SqlPath .\scripts\p0\public-routine-live-evidence.sql -LiveRoutineEvidence
powershell.exe -NoProfile -ExecutionPolicy Bypass -File .\scripts\p0\validate-catalog-snapshot-readonly.ps1 -SqlPath .\scripts\p0\public-foreign-key-risk-live-evidence.sql -LiveForeignKeyEvidence
```
