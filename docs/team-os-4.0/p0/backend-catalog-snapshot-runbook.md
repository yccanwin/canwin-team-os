...
> SQL：`scripts/p0/catalog-snapshot.sql`、`scripts/p0/public-table-live-evidence.sql`、`scripts/p0/public-routine-live-evidence.sql`
...
当前 `public-table-live-evidence.sql` 和 `public-routine-live-evidence.sql` 已分别通过 Supabase 只读查询执行一次，标准化结果保存在 `public-table-live-evidence.json`、`public-routine-live-evidence.json`。前者证明 103/103 表的 owner、行数估计、RLS、有效 GRANT、229 条策略、29 个触发器、248 个索引、309 个外键和 catalog 可见依赖；后者登记 162/162 函数签名的 owner、ACL、`search_path`、提权标志、触发器引用和定义指纹。两类证据的业务行读取=0、写入=0，函数正文返回=0；完整函数正文依赖、策略表达式和原始连接输出仍不进入仓库。
