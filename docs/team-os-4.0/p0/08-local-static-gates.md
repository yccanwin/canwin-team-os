...
- scripts/p0/ci-database-test-contract.json、scripts/p0/ci-runtime/supabase/config.toml 与 scripts/p0/verify-ci-database-contract.mjs：冻结临时本地数据库、1 份基线、69 个迁移、26 个 SQL 测试、4 项 catalog 对账、Supabase CLI 2.109.1、Postgres 17 和零仓库密钥边界；22 个负向自检拒绝清单、连接、版本、全栈误启动、`DO` 美元引用词法粘连、把限制策略误当写授权、导入历史直接写权限、过期迁移链预期、失败证据删除或 G0 状态漂移。
...
- CI 数据库边界自检：合同 negative=22/22、runner negative=6/6、databaseCalls=0；
