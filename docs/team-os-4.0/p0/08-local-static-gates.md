...
- scripts/p0/ci-database-test-contract.json、scripts/p0/ci-runtime/supabase/config.toml 与 scripts/p0/verify-ci-database-contract.mjs：冻结临时本地数据库、1 份基线、69 个迁移、26 个 SQL 测试、4 项 catalog 对账、Supabase CLI 2.109.1、Postgres 17 和零仓库密钥边界；冻结测试引用的 52 个函数/视图定义和其中 28 个重复定义对象，要求直接正向源码断言存在于对象最后一次定义所在迁移；25 个负向自检拒绝清单、连接、版本、全栈误启动、`DO` 美元引用词法粘连、把限制策略误当写授权、函数/视图正文格式敏感或错指早期迁移、导入历史直接写权限、过期迁移链预期、失败证据删除或 G0 状态漂移。
...
- CI 数据库边界自检：合同 definitions=52、redefined=28、negative=25/25，runner negative=6/6、databaseCalls=0；
