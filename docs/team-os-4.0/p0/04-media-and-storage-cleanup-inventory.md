...
> 状态：源码静态盘点完成；线上对象清单、备份恢复和隔离攻击验证未完成
...
`photos/achievements.images/assets/timeline/attachments/goal_update/avatar` 统一进入 `read_only`；禁止申请新上传意图。历史读取、30 天兼容、最终归档和孤儿对象处理分别列单。
...
- 线上 bucket/policy/object 完整清单和孤儿对象统计。
- 数据库与 Storage 文件分别备份、分别恢复的成功证据。
- 合法两槽和非法攻击矩阵的隔离环境结果。
- 旧页面只读后，历史媒体链接和签名 URL 的运行时验证。
