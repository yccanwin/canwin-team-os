...
> 状态：P1 工单和接口已冻结；历史失败运行和历史exact70续验资格均原样保留。ACL/原子候选 CI run [`29726897764`](https://github.com/yccanwin/canwin-team-os/actions/runs/29726897764) 已首错停止：Linux完成迁移71/71及SQL17/18后发现审计断言把正确的5/1/1合计7误写为6，Windows static19/19、local12/12通过。新候选已改为按动作分别锁定5/1/1，本地门禁通过、数据库CI待验；`remote=false`、`dbPush=false`。全量对账和六类账号页面验收仍待完成，G1=false，整体验收仍为25%。
...
- 正式续验 run `p1-resume-20260719T193911279Z-ea6ed9385d` 的原现场继续保留。第71迁移 LF SHA256 为 `1bb13f29fc0f5512bd00115dc1c953a2c3aaa0ec21522b1cc8cbb45a18a5cdc0`；notification/P1 access测试SHA分别为 `a3d87069899b986b191bc21826f5e23c65fe4734066e52adc4e14753c9e6e5a3`、`c598b4e4ed3c7e26d9411cb4084685bea1233f47ae969c2685e048f480dac09e`，runtime contract/runner/validator SHA分别为 `0c0ccc0f72775ee1e6c6d46771846de88c5054a20b3f75b30a77db40ced08991`、`690dd490c4c66c84ed1d01654c77cdb6cf8ca068b6625c9f44ed78e32c0098ad`、`bc237b9b35ca6954a9c4cd6e905ff7a02608ad3509fcd439ba64a5a4882b8da3`。CI run `29726897764`/Linux job `88301987239` 的审计断言首错与Windows job `88301987280`成功均已写入合同；失败运行不重跑，当前新候选数据库CI待签署，remote/db push关闭，G1=false、25%。
- 旧RPC应用兼容性已通过：管理员页面改走 `admin-members` 服务端入口，Edge 写链改用v1岗位/主管RPC，三个退役写RPC静态调用为0，app-shell 99/99。原子兼容静态证据为mapping5/5、rollback2/2、same-team4/4、审计分项5/1/1、private definition snapshots3/3、repairGate负控5/5和atomic-only负控2/2；当前状态是 `static-passed-prior-database-ci-failed-preserved-new-candidate-pending`，不得签G1或计为30%。尚缺新独立数据库CI、候选正式应用、29份全量对账及六类账号页面验收，remote/db push继续关闭。
