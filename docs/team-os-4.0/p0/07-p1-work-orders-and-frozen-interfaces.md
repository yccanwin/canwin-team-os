...
> 状态：P1 工单、物理 RPC 名、字段白名单和测试身份编号已由监理冻结；执行仍锁定，只有 G0 通过后才允许进入编码。
...
- GitHub CI 实际执行数据库、权限和业务测试；已保留15个独立候选的远端证据，最新候选已通过Windows门禁、临时数据库启动、基线、69/69迁移和25/26项SQL，但尚无26/26测试及4/4 catalog对账的完整全绿证据，故状态仍为 `pending`。
- G0 联合签署；冻结工单不等于 P1 已经开工，更不等于 G1 岗位权限验收完成。
