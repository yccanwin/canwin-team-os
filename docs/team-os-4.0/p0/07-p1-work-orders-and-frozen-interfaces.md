...
> 状态：P1 工单和接口已冻结；历史失败运行和历史exact70续验资格均原样保留。正式续验 run `p1-resume-20260719T193911279Z-ea6ed9385d` 已在第6个 SQL `notification_core.sql` 发现 worker RPC 暴露并首错停止，完成5/27 SQL、5份逐项快照和6/29完整快照。当前单一pre70/post71候选冻结为第71迁移ACL目标6/实际变化4，加一个私有成员授权函数定义变化；原子兼容静态通过、数据库CI待验，27项逐项和29份完整快照不变。新CI签署前 `remote=false`、`dbPush=false`。全量对账和六类账号页面验收仍待完成，G1=false，整体验收仍为25%。
...
- 正式续验 run `p1-resume-20260719T193911279Z-ea6ed9385d` 已执行1次并首错停止：前5项SQL和5份逐项快照通过，第6项 `notification_core.sql` 报 `Notification worker RPC exposed`，累计6/29完整快照、Storage归档1次。preflight SHA256 `e0ea653d3a411cc9baafbd4b98e7d6d458b99316e8da93a1db1600a21e2dc36a`，failure SHA256 `576a11005285cd708adca5b3486e0b929ace8d97fc3cc3284d657b57519b91ad`；现场仅两文件、无success证据，持久写入和生产读写均0，目标保留、未重试、未清理。第71迁移 LF SHA256 为 `1bb13f29fc0f5512bd00115dc1c953a2c3aaa0ec21522b1cc8cbb45a18a5cdc0`，包含ACL目标6/实际变化4和私有定义变化1；notification/P1 access测试SHA分别为 `a3d87069899b986b191bc21826f5e23c65fe4734066e52adc4e14753c9e6e5a3`、`c4823724a65047b0e67af6ba62c954acf3085d70ffbbda1c5e1a0be23ce94dfb`，runtime contract/runner/validator SHA分别为 `421f78b398eb259d2a3fa43836e7a3ff9f2c70527ad88af1896b0d0a4e118d08`、`9f04478e3f218dad16eec3660919dfedabb3e69ad2a2c7522b7caa396a010620`、`168bd3e03b1fc3e9536fc74d353b10e01a2b51c1ff1ff8a3e9eb5d3930233577`。27项SQL和29份完整快照不变，数据库CI签署前remote/db push关闭，G1=false、进度25%。
- 旧RPC应用兼容性已通过：管理员页面改走 `admin-members` 服务端入口，Edge 写链改用v1岗位/主管RPC，三个退役写RPC静态调用为0，app-shell 99/99。原子兼容静态证据为mapping5/5、rollback2/2、same-team4/4、private definition snapshots3/3、repairGate负控5/5和atomic-only负控2/2；当前状态是 `static-passed-database-ci-pending`，不得签G1或计为30%。尚缺新独立数据库CI、候选正式应用、29份全量对账及六类账号页面验收，remote/db push继续关闭。
