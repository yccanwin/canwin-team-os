...
> 状态：P1 工单和接口已冻结；历史失败运行均原样保留且未重跑，换行修复后的独立 CI run `29696529290` 已双平台全绿。前次正式隔离尝试已完成 P1 迁移应用，但在首个 SQL 校验误报处停止；续验候选的最终本地预资格已冻结为 exact70、27项逐项、29份完整快照、2次 Storage 内容复核和6份签名制品，四种夹具模式覆盖4/4、runner validator 97/97。旧应用入口永久关闭，续验入口仍关闭且未签署 CI HEAD。续验未执行，开始时仍须先证明 exact70 当前快照；全量对账和六类账号页面验收仍待完成。当前为 `post-apply verification candidate pending`，candidateRemote=false、resumeRemote=false、resumeSigned=null，G1=false，整体验收仍为25%。
...
- 续验候选已完成最终纯本地预资格冻结：access-control 回滚夹具 LF SHA `31fa286b318ad2b24e2d956005c4a5fcc9b0fddfd0269be029330d5c1c3e43f8`；resume-only runner SHA `6bea4b4ff000d140ad783f0dadf7813d4079c1b16169651ab8e256858c3bba98`；isolated runtime contract SHA `5a9e98cd0e80541c5de09c384ddb26b267eb0ed0ba4dcee61bb20f9a89fd01ca`；runner validator SHA `842a82a5d11216c200a40efe222135d76f158e3eb11aae9e593e255f06872ae6`，静态断言97/97。预资格锁定 exact70、27项SQL逐项完整快照、总计29份完整快照（初始1+逐项27+新凭据最终1）、Storage初末2次内容归档、6份加密签名制品，以及关键金额5项、原始账本9项、库存3项、Auth/Session隔离、逐表/Auth/Schema内容指纹和前后 canonical SHA 相等。只允许 `profile_access_roles` 的已授权角色/assignment_kind 变化与 `feature_flags` 的P1单团队行，未知差异0；同时明确旧P0制品只有行数且 `targetAfterSha256=null`，不能代替P1初末内容指纹。fixture pattern覆盖 `p1/access` 邮箱与 `d400/d510` ID 共4/4。旧 migration apply 入口 `candidate.remoteExecutionAllowed=false`，续验入口 `resume.remoteExecutionAllowed=false`、`signedCiHeadSha=null`，禁止db push、预期持久化写入0。该状态仍只是本地预资格；续验远端执行0，未取得当前 exact70、27/27、4/4 catalog或页面验收，G1=false、进度25%。
