...
> 状态：P1 工单和接口已冻结；历史失败运行和历史exact70续验资格均原样保留。ACL/原子候选 CI run [`29726897764`](https://github.com/yccanwin/canwin-team-os/actions/runs/29726897764) 已首错停止且未重跑，run [`29733854344`](https://github.com/yccanwin/canwin-team-os/actions/runs/29733854344) 仅作为历史成功证据保留。正式命令 `p1-acl-repair-20260720T104323349Z-4fa8de78a8` 的 `db-push-dry-run` 首错现场继续保留，正式/db push尝试0、持久写0、生产读写0。新候选 CI run [`29738966326`](https://github.com/yccanwin/canwin-team-os/actions/runs/29738966326)（HEAD `070c2e4`）双平台成功：Linux job `88340968144`、Windows job `88340968119`；当前重新开放独立测试项目一次正式 ACL repair 资格，生产禁止。G1=false，整体验收仍为25%。
...
- 正式 ACL repair 命令 `p1-acl-repair-20260720T104323349Z-4fa8de78a8` 在 `db-push-dry-run` 首错停止，错误为“dry-run 未精确枚举唯一签名迁移71”。证据目录 `D:\CanWin-Team-OS-4.0-P1-Validation\p1-acl-repair-20260720T104323349Z-4fa8de78a8` 仅有 `failure.json`，SHA256 `16373794dd745ad86422bb59f3966933532cb0bf073251963b519c2b8e367e73`；`formalAttemptStarted=false`、正式尝试0、db push尝试0、持久写0、生产读写0，目标保留、未重试、未清理。旧证据只证明dry-run枚举门失败；后续本地只读诊断确认解析器未识别当前CLI输出。修复后的 runner 自测、验证器68/68、本地 static19/19、local12/12 已通过；失败现场继续保留，新 run `29738966326` 已重新签署当前资格。
- 旧RPC应用兼容性已通过：管理员页面改走 `admin-members` 服务端入口，Edge 写链改用v1岗位/主管RPC，三个退役写RPC静态调用为0，app-shell 99/99。原子兼容历史CI证据为mapping5/5、rollback2/2、same-team4/4、审计分项5/1/1、private definition snapshots3/3、repairGate负控5/5和atomic-only负控2/2；run `29733854344` 的成功记录继续保留，新 run `29738966326`（HEAD `070c2e4`，Linux `88340968144`、Windows `88340968119`）双平台成功，当前 `remote/dbPush=true` 仅开放独立测试项目一次正式 ACL repair。不得签G1或计为30%；尚缺正式应用、29份全量对账及六类账号页面验收。
