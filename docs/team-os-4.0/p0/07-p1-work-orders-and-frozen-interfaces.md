...
- 旧RPC应用兼容性已通过：管理员页面改走 `admin-members` 服务端入口，Edge 写链改用v1岗位/主管RPC，三个退役写RPC静态调用为0，app-shell 99/99。原子兼容证据为mapping5/5、rollback2/2、same-team4/4、审计分项5/1/1、private definition snapshots3/3、repairGate负控5/5和atomic-only负控2/2；当前状态是 `passed`。`remote/dbPush=true` 只为一次独立测试项目 ACL repair，不得签G1或计为30%；尚缺正式应用、29份全量对账及六类账号页面验收。
