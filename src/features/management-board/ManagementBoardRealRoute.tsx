...
export default function ManagementBoardRealRoute() {
  ...
  const submitResolution = async () => {
    ...
    try {
      await dataSource.resolveException({ itemType: selected.itemType, entityId: selected.entityId, ownerId: selected.ownerId, dueAt: new Date(resolutionDueAt).toISOString(), note: resolutionNote.trim() })
  ...
  return <section className="min-h-screen bg-slate-50 p-4 md:p-6">
    <header className="mb-5"><p className="text-sm font-medium text-indigo-600">真实数据入口</p><h1 className="text-2xl font-bold text-slate-900">主管异常与临门商机板</h1><p className="mt-1 text-sm text-slate-500">展示真实异常和已报价且0–7天内决策商机；处置只调用服务器RPC。</p></header>
