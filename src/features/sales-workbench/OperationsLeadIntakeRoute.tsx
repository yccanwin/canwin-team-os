...
export default function OperationsLeadIntakeRoute() {
  ...
  const requestPreview = async () => {
    if (!draft.customerName.trim() || !draft.phone.trim()) return setMessage({ tone: 'error', text: '客户/门店名称和联系电话不能为空。' })
  ...
  const submit = async () => {
    ...
    setBusy(true); setMessage(null)
    const { data, error } = await supabase.rpc('submit_operations_lead', { p_customer_name: draft.customerName.trim(), p_contact_name: draft.contactName.trim() || null, p_phone: draft.phone.trim(), p_region_text: draft.regionText.trim() || null, p_address: draft.address.trim() || null, p_notes: draft.notes.trim() || null, p_raw_text: rawText })
    setBusy(false)
  ...
  return <main className="oli-page">
    ...
    <section className="oli-card"><div className="oli-section-title"><UserRoundCheck size={22}/><div><h2>核对线索信息</h2><p>识别结果可以修改，修改后请重新预览分配</p></div></div>
      <div className="oli-grid">
        <label><span>客户/门店名称 *</span><input value={draft.customerName} onChange={(e) => update('customerName', e.target.value)} placeholder="客户或门店名称"/></label><label><span>联系人</span><input value={draft.contactName} onChange={(e) => update('contactName', e.target.value)} placeholder="联系人姓名"/></label><label><span>联系电话 *</span><input value={draft.phone} inputMode="tel" onChange={(e) => update('phone', e.target.value)} placeholder="手机或座机"/></label>
        <label><span>所在区域</span><input value={draft.regionText} onChange={(e) => update('regionText', e.target.value)} placeholder="区县/街道"/><small>{matchedRegion ? `已匹配：${matchedRegion.name}` : '未匹配时将进入公海'}</small></label><label className="oli-wide"><span>详细地址</span><input value={draft.address} onChange={(e) => update('address', e.target.value)} placeholder="街道、门牌号或商场楼层"/></label><label className="oli-wide"><span>备注/需求</span><textarea value={draft.notes} onChange={(e) => update('notes', e.target.value)} rows={3} placeholder="开业时间、产品需求、客户承诺等"/></label>
      </div>
