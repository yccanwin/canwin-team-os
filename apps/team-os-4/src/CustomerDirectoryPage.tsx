...
import type { Customer, CustomerDirectory } from './domain/customer'
...
export function CustomerDirectoryPage({ user }: { user: AuthenticatedWorkspace }) {
  ...
  return <section className="workspace" data-testid="customer-directory-list"><p className="eyebrow">销售业务</p><h1>客户、品牌与门店</h1><ol className="work-item-list">{data.customers.map((customer) => { const brandIds = data.brands.filter((brand) => brand.customerId === customer.id).map((brand) => brand.id); return <li key={customer.id} data-testid="customer-row"><button className="customer-row-button" onClick={() => setSelected(customer)}><div><strong>{customer.name}</strong><span>{customer.region} · 品牌 {brandIds.length} · 门店 {data.stores.filter((store) => brandIds.includes(store.brandId)).length}</span></div><StatusBadge tone="success">客户</StatusBadge></button></li> })}</ol>{selected && <Customer360Drawer customer={selected} brands={data.brands} stores={data.stores} onClose={() => setSelected(undefined)} />}</section>
}
